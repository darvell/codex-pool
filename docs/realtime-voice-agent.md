...
## Codex Desktop and `codex app-server`

Codex app-server owns a different WebRTC flow from the standalone Agents SDK example above. Configure both signaling transports so the SDP call creation and its sideband WebSocket pass through the pool:

```toml
experimental_realtime_webrtc_call_base_url = "https://codex.ppflix.net/v1"
experimental_realtime_ws_base_url = "https://codex.ppflix.net/v1"
realtime.version = "v3"
realtime.transport = "webrtc"
```

Then enable the experimental realtime feature:

```bash
codex features enable realtime_conversation
```

Codex posts its SDP offer to the call base URL and joins the returned call through the WebSocket base URL. The pool pins the returned `rtc_...` call ID to the account that created it so both requests use the same pooled identity. Setup scripts generated by `/setup/codex/...` install these settings automatically.
